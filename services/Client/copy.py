...
class StockPortfolio:
    ...
    def fill_missing_dates(self):
        ...
        # print(self.stock_price.index)
        # print(type(self.stock_price.index))
    ...
    def plot_rate_of_return_history(self):
        ...
        #수익률 계산
        spy_price = (spy_price[0] - spy_price) / spy_price[0]
        # cash_plus_stock_values 리스트의 첫 번째 값을 가져옴
        initial_value = cash_plus_stock_values[0]

        # 리스트의 각 요소에 대해 연산을 수행하여 새로운 리스트 생성
        cash_plus_stock_values = [(initial_value - value) / initial_value for value in cash_plus_stock_values]

        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=dates, y=cash_plus_stock_values, mode='lines+markers', name='Cash + Stocks',
        ...
        for date, annotations in buy_annotations.items():
            fig.add_trace(go.Scatter(
                x=[date], y=[self.daily_history[date.strftime('%Y-%m-%d')]['cash_plus_stock']],
        ...
        for date, annotations in sell_annotations.items():
            fig.add_trace(go.Scatter(
                x=[date + timedelta(minutes=400)], y=[self.daily_history[date.strftime('%Y-%m-%d')]['cash_plus_stock']],
        ...
        fig.update_layout(title='Price',
                        ...
                        xaxis=dict(
                            title=dict(font=dict(size=30))  # Increase the x-axis title font size
                        ),
                        yaxis=dict(
                            title=dict(font=dict(size=30))  # Increase the y-axis title font size
        ))
